...
function setup() {
  ...
  const state = normalize({ language: 'zh-CN' }, now); state.watchedMs = 1800000;
...
test('floating panel shows selected mode, used and remaining; collapse and side switch do not affect allowance', () => {
  ...
  assert.equal(overlay.remaining.textContent, '00:30:00'); assert.equal(overlay.detail.textContent, '已看 00:30:00 / 01:00:00');
  assert.equal(overlay.progress.value, 50); overlay.toggle.click(); assert.equal(overlay.body.hidden, true);
  assert.equal(overlay.compact.textContent, '剩余 00:30:00'); assert.equal(overlay.toggle.getAttribute('aria-expanded'), 'false');
...
test('full-screen container receives overlay and open dialog; settings action and failures are handled', async () => {
  ...
  assert.equal(f.overlay.error.hidden, false); assert.match(f.overlay.error.textContent, /无法打开设置/); f.overlay.dispose();
});
test('session dialog uses session total instead of daily total', () => {
  ...
  assert.equal(overlay.mode.textContent, '临时观看额度'); assert.match(overlay.message.textContent, /本次设置时刻/);
  assert.equal(overlay.summary.textContent, '已观看 00:01:00 · 上限 00:01:00'); overlay.dispose();
});
test('redirect mode navigates once without a dialog; hidden pages do not navigate and failures fall back to dialog', () => {
  ...
  assert.match(overlay.error.textContent, /无法跳转/); overlay.dispose();
});
...
test('exempt overlay suppresses both actions, exposes toggle, and refreshes on video identity changes', async () => {
  ...
  state.excludedVideos = [{ id, title: '课程' }]; state.watchedMs = 3600000;
  overlay.setVisible(true); overlay.update(state);
  assert.equal(overlay.dialog.open, false); assert.equal(overlay.mode.textContent, '不计时');
  assert.equal(overlay.excludeButton.textContent, '恢复计时');
  ...
  assert.equal(overlay.excludeButton.textContent, '不计时'); overlay.dispose();
});
test('overlay and open dialog switch language live without a new alert or changing collapse state', () => {
  const { overlay, state } = setup(); overlay.setVisible(true); state.watchedMs = 3600000; overlay.update(state);
  ...
  overlay.showError('观看时间暂时无法保存，请刷新页面重新连接。'); assert.match(overlay.error.textContent, /Reload/);
  overlay.update({ ...state, language: 'zh-CN' }); assert.equal(overlay.dismiss.textContent, '知道了');
  assert.match(overlay.error.textContent, /刷新/); assert.equal(overlay.dialog.shows, shows); overlay.dispose();
});
